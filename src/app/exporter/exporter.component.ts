...
export class ExporterComponent implements OnInit {
  ...
  getFilteredData() {
    ...
    this.service.getFilteredData(dateReception1, dateReception2).subscribe(
      (res: any) => {
        this.silos = res;
      },
      (error) => {
  ...
  downloadPdf() {
    ...
    this.service.downloadPdf(dateReception1, dateReception2).subscribe(
      (pdfBlob) => {
  ...
  exportSilosTOExcel() {
    ...
    if (dateReception1 && dateReception2) {
      try {
        this.service.exportSilos(dateReception1, dateReception2);
      } catch (error) {
        this.toster.error("Erreur lors de l'exportation du fichier Excel.");
      }
    } else {
      this.toster.error(
        'Erreur lors du téléchargement du fichier Excel : Dates invalides.'
      );
    }
  }
}
